...
- 默认脚本语言是 `python`。
- 只有非常薄的命令胶水才使用 `shell`。
- 只有在脚本需要直接复用仓库 TypeScript 模块、类型或 Bun 运行时能力时，才优先使用 `bun/ts`。
- `ruby` 不作为默认脚本语言；除非存在明确工具链收益，否则不要继续增加 Ruby 脚本。
- 如果脚本开始解析结构化数据、批量读写文件或生成内容，不要继续硬用 shell。
